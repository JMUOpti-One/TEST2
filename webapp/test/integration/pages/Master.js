				...
				actions : {
										...
										var sCurrentId,
											aItemsNotInTheList = aEntityData.filter(function (oObject) {
												return !oList.getItems().some(function (oListItem) {
													return oListItem.getBindingContext().getProperty("EntryB1EDI") === oObject.EntryB1EDI;
												});
											});

										if (!aItemsNotInTheList.length) {
											// Not enough items all of them are displayed so we take the last one
											sCurrentId = aEntityData[aEntityData.length - 1].EntryB1EDI;
										} else {
											sCurrentId = aItemsNotInTheList[0].EntryB1EDI;
										}
					...
					iRememberTheListItem : function (oListItem) {
						...
						this.getContext().currentItem = {
							...
							id: oBindingContext.getProperty("EntryB1EDI"),
							title: oBindingContext.getProperty("EntryB1EDI")
						};
					}
				},
				...
				assertions : {
					...
					theListHeaderDisplaysZeroHits : function () {
						return this.waitFor({
							...
							success : function () {
								Opa5.assert.ok(true, "The list header displays 'Headers (0)'");
							},
					...
					theListShouldBeSortedAscendingOnName : function () {
						return this.theListShouldBeSortedAscendingOnField("EntryB1EDI");
					},
